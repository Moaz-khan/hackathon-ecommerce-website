import React from "react";
import Rating from "../ui/Rating";
import Image from "next/image";
import Link from "next/link";
import { Product } from "@/types/product.types";

type ProductCardProps = {
  data: Product;
};

// Helper function to calculate the discounted price
const calculateDiscountedPrice = (
  price: number,
  discount: { percentage: number; amount: number },
) => {
  if (discount.percentage > 0) {
    return Math.round(price - (price * discount.percentage) / 100);
  }
  if (discount.amount > 0) {
    return price - discount.amount;
  }
  return price;
};

// Helper function to render the discount tag
const renderDiscountTag = (discount: {
  percentage: number;
  amount: number;
}) => {
  if (discount.percentage > 0) {
    return (
      <span className="font-medium text-[10px] xl:text-xs py-1.5 px-3.5 rounded-full bg-[#FF3333]/10 text-[#FF3333]">
        {`-${discount.percentage}%`}
      </span>
    );
  }
  if (discount.amount > 0) {
    return (
      <span className="font-medium text-[10px] xl:text-xs py-1.5 px-3.5 rounded-full bg-[#FF3333]/10 text-[#FF3333]">
        {`-$${discount.amount}`}
      </span>
    );
  }
  return null;
};

// Helper function to render the original price with a strikethrough
const renderOriginalPrice = (
  price: number,
  discount: { percentage: number; amount: number },
) => {
  if (discount.percentage > 0 || discount.amount > 0) {
    return (
      <span className="font-bold text-black/40 line-through text-xl xl:text-2xl">
        ${price}
      </span>
    );
  }
  return null;
};

const ProductCard = ({ data }: ProductCardProps) => {
  const discountedPrice = calculateDiscountedPrice(data.price, data.discount);

  return (
    <Link
      href={`/shop/product/${data.id}/${data.title.split(" ").join("-")}`}
      className="flex flex-col items-start aspect-auto">
      <div className="bg-[#F0EEED] rounded-[13px] lg:rounded-[20px] w-full lg:max-w-[295px] aspect-square mb-2.5 xl:mb-4 overflow-hidden">
        <Image
          src={data.srcUrl}
          width={295}
          height={298}
          className="rounded-md w-full h-full object-contain hover:scale-110 transition-all duration-500"
          alt={data.title}
          priority
        />
      </div>
      <strong className="text-black xl:text-xl">{data.title}</strong>
      <div className="flex items-end mb-1 xl:mb-2">
        <Rating
          initialValue={data.rating}
          allowFraction
          SVGclassName="inline-block"
          emptyClassName="fill-gray-50"
          size={19}
          readonly
        />
        <span className="text-black text-xs xl:text-sm ml-[11px] xl:ml-[13px] pb-0.5 xl:pb-0">
          {data.rating.toFixed(1)}
          <span className="text-black/60">/5</span>
        </span>
      </div>
      <div className="flex items-center space-x-[5px] xl:space-x-2.5">
        <span className="font-bold text-black text-xl xl:text-2xl">
          ${discountedPrice}
        </span>
        {renderOriginalPrice(data.price, data.discount)}
        {renderDiscountTag(data.discount)}
      </div>
    </Link>
  );
};

export default ProductCard;
